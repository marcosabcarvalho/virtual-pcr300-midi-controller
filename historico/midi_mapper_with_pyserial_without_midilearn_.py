...
WHITE_KEYS = 28           # 4 oitavas * 7 teclas brancas por oitava = 28 teclas brancas (C2 a B5)
...
class PCR300Virtual(tk.Tk):
    def __init__(self, serial_port='/dev/ttyUSB0', baudrate=31250):
        ...
        self.geometry("2500x1000")  # Ajustado para acomodar mais controles e 4 oitavas
        ...
        self.controller_to_virtual_map = {note: note for note in range(36, 84)}  # C2 (36) até B5 (83)
    ...
    def create_virtual_keyboard(self, parent):
        """
        Cria um teclado virtual mais realista com 4 oitavas de teclas brancas e pretas.
        """
        key_width = 30
        key_height = 150
        black_key_width = key_width * 0.6
        black_key_height = key_height * 0.6
        num_octaves = WHITE_KEYS // 7  # 4 oitavas
        remaining_keys = WHITE_KEYS % 7  # 0, já que 4 * 7 = 28
    ...
    def handle_midi_message(self, message):
        ...
        if message.type == "control_change":
            ...
            if virtual_note:
                print(f"Mapeando nota {message.note} para {virtual_note}")
    ...
    def press_virtual_key(self, virtual_note, velocity=100):
        ...
        """
        if virtual_note in self.white_key_map:
            rect_id = self.white_key_map[virtual_note]
            self.kb_canvas.itemconfig(rect_id, fill="yellow")  # Cor para tecla branca pressionada
            print(f"Tecla branca {virtual_note} pressionada.")
        elif virtual_note in self.black_key_map:
            rect_id = self.black_key_map[virtual_note]
            self.kb_canvas.itemconfig(rect_id, fill="red")  # Cor para tecla preta pressionada
            print(f"Tecla preta {virtual_note} pressionada.")
        else:
            print(f"Nota {virtual_note} fora do range do teclado virtual.")
            return
    ...
    def release_virtual_key(self, virtual_note, velocity=100):
        ...
        if virtual_note in self.white_key_map:
            ...
            print(f"Tecla branca {virtual_note} solta.")
        elif virtual_note in self.black_key_map:
            rect_id = self.black_key_map[virtual_note]
            self.kb_canvas.itemconfig(rect_id, fill="black")  # Cor padrão para tecla preta
            print(f"Tecla preta {virtual_note} solta.")
        else:
            print(f"Nota {virtual_note} fora do range do teclado virtual.")
            return
