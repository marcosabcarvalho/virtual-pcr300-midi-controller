...
class CircularKnob(tk.Canvas):
    ...
    def update_value(self, x, y):
        dx = x - self.center
        dy = self.center - y  # Inverter y para que o ângulo seja calculado corretamente
        angle = math.atan2(dy, dx)
        angle_deg = math.degrees(angle)
        
        # Ajustar o ângulo para o intervalo desejado (135 a -135 graus)
        if angle_deg < -135:
            angle_deg += 360
        if angle_deg > 135:
            angle_deg -= 360
        
        # Mapear o ângulo para o valor
        if angle_deg < -135:
            angle_deg = -135
        elif angle_deg > 135:
            angle_deg = 135
        # Normalizar para o intervalo [0, 270]
        normalized_angle = angle_deg + 135
        # Mapear para o valor
        value = self.min_val + (normalized_angle / 270) * (self.max_val - self.min_val)
...
class PCR300Virtual(tk.Tk):
    ...
    def create_main_interface(self):
        ...
        for i in range(18):
            ...
            # Define a callback para cada knob
            knob = CircularKnob(
